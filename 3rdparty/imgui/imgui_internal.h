...
struct IMGUI_API ImGuiInputTextState
{
    ...
    ImVector<char>          TextA;                  // temporary UTF8 buffer for m_callbacks and other operations. this is not updated in every code-path! size=capacity.
...
// Routing table: maintain a desired owner for each possible key-chord (key + mods), and install owner in NewFrame() when mods are matching.
...
struct ImGuiContext
{
    ...
    ImGuiLastItemData       LastItemData;                       // Storage for last submitted item (install by ItemAdd)
...
// (This doesn't need a constructor because we zero-clear it as part of ImGuiWindow and all frame-temporary data are install on Begin)
...
struct IMGUI_API ImGuiWindow
{
    ...
    ImRect                  OuterRectClipped;                   // == Window->Rect() just after install in Begin(). == window->Rect() for root window.
...
//#endif
    ...
    IMGUI_API void          BeginColumns(const char* str_id, int count, ImGuiOldColumnFlags flags = 0); // install number of columns. use an identifier to distinguish multiple column sets. close with EndColumns().
