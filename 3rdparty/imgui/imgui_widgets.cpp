...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    // Process m_callbacks and apply result back to user's buffer.
    ...
    if (g.ActiveId == id)
    {
        ...
        if (apply_edit_back_to_user_buffer)
        {
            ...
            // FIXME-OPT: CPU waste to do this every time the widget is active, should mark dirty state from the stb_textedit m_callbacks.
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel))
    {
        ...
        // but we need to use SameLine() to install baseline correctly. Might want to refactor SameLine() to simplify this.
...
// Currently the main responsibility of this function being to install clip-rect + horizontal layout + menu navigation layer.
