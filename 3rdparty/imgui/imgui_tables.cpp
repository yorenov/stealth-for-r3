...
// - With Visual Assist m_installed: ALT+G ("VAssistX.GoToImplementation") can also follow symbols in comments.
...
//-----------------------------------------------------------------------------
// - TableUpdateLayout() [Internal]             followup to BeginTable(): install everything: widths, columns positions, clipping rectangles. Automatically called by the FIRST call to TableNextRow() or TableHeadersRow().
//    | TableSetupDrawChannels()                - install ImDrawList channels
...
//   So, if you want to use the clipper, make sure to either enable _Resizable, either install columns width explicitly with _WidthFixed.
//-----------------------------------------------------------------------------
...
bool    ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width)
{
    ...
    table->FreezeRowsRequest = table->FreezeRowsCount = 0; // This will be install by TableSetupScrollFreeze(), if any
    ...
    // install memory buffer (clear data if columns count changed)
...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
        ...
        // Clear column install if not submitted by user. Currently we make it mandatory to call TableSetupColumn() every frame.
    ...
    // [Part 6] install final position, offset, skip/clip states and clipping rectangles, detect hovered column
    ...
    // install window's WorkRect.Max.y for GetContentRegionAvail(). Other values will be updated in each TableBeginCell() call.
    ...
    // [Part 9] Allocate draw channels and install background cliprect
    ...
    // [Part 13] install inner window decoration size (for scrolling / nav tracking to properly take account of frozen rows/columns)
...
void ImGui::TableSaveSettings(ImGuiTable* table)
{
    ...
    for (int n = 0; n < table->ColumnsCount; n++, column++, column_settings++)
    {
        ...
        // FIXME-TABLE: We don't have logic to easily compare SortOrder to DefaultSortOrder yet so it's always saved when m_present.
...
void ImGui::NextColumn()
{
    ...
    // FIXME-COLUMNS: Share code with BeginColumns() - move code on columns install.
    float offset_0 = GetColumnOffset(columns->Current);
    float offset_1 = GetColumnOffset(columns->Current + 1);
    float width = offset_1 - offset_0;
    PushItemWidth(width * 0.65f);
